...
import WordCountBadge from "../../utils/WordCountBadge.tsx";
...
const WriteTestMenu: React.FC = () => {
    ...
    if (submitted) return (
        <div className="flex h-screen flex-col items-center justify-center gap-4 text-base-content">
            ...
            {/* FIX: was <h2> inside Button's label prop — invalid HTML (block in inline) */}
            ...
            <Button
                label={isLast
                    ? "View results"
                    : `Continue to ${SKILL_LABEL[nextStep?.key ?? ""] ?? "next"} section`
                }
                variant="primary"
        ...
        <div className="flex flex-col h-screen bg-slate-800 overflow-hidden w-full">
            ...
            <main className="flex flex-1 overflow-hidden">
                ...
                <div className="w-1/2 overflow-y-auto p-6 border-r border-base-300 bg-base-50">
                    ...
                    <p className="text-sm text-base-content mb-4 italic p-2 bg-base-200/30 rounded">{currentTask.description}</p>
                    <p className="text-sm text-base-content mb-6 p-2 bg-base-200/20 rounded">{currentTask.question}</p>
                    {currentTask.diagram_url !== null && currentTask.taskType === 'map_description' ? (
