...
import {
    ...
    Mic, Play, Square, Volume2, Home, MicOff,
...
import type { SpeakTest, Parts } from "../../../spikingTask/speak.ts";
...
const SpeakTestMenu: React.FC = () => {
    const { goNext, goFinish, isLast, nextStep } = useExamNav();
    // FIX: removed `useAuthStore` — `user` was only used to send user?.id in the
    // request body, which the backend ignores (reads from req.user._id via JWT instead)
    ...
    if (submitted) {
        ...
        return (
            <div className="flex h-screen flex-col items-center justify-center gap-6 bg-base-100 text-base-content px-6">
                ...
                <Button onClick={() => isLast ? goFinish() : goNext() } >
                    {isLast
                        ? <><Home size={15} /> Finish exam</>
                        : <>Continue to {SKILL_LABEL[nextStep?.key ?? ""] ?? "next"} <ArrowRight size={15} /></>
                    }
                </Button>
